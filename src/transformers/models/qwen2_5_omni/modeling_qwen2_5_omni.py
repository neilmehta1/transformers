...
def save_io(func):
    """
    Decorator to save inputs and outputs of the decorated method.
    This will save the inputs and outputs as PyTorch tensors in a directory.
    IO is saved only for the first time the decorated function is called
    for a specific class and function combination *within the current process*.
    Subsequent calls in the same process will not save IO.
    If the save directory exists from a previous process, it will be overwritten
    on the first call in the new process.
    """
    # This set is part of the closure of the 'wrapper' function.
    # It will persist across calls to the same decorated function instance.
    # To make it "once per (class, function_name) per process", it needs to be
    # accessible/shared by all wrappers generated by this decorator.
    # A simple way is to attach it to the 'save_io' function itself.
    if not hasattr(save_io, '_io_saved_this_process'):
        save_io._io_saved_this_process = set()
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if func_identifier in save_io._io_saved_this_process:
            # IO already saved for this function in this process, skip.
            # print(f"IO for {func_identifier} already saved in this process. Skipping save.")
            return func(self, *args, **kwargs)

        # First call in this process for this function.
        # Mark as saved for this process.
        save_io._io_saved_this_process.add(func_identifier)

        # Find the mlx_a2a root and determine the save directory
        try:
            repo_root = find_mlx_a2a_root()
        except RuntimeError as e:
            # print(f"Warning: {e}. Skipping IO capture for {func_identifier}.")
            return func(self, *args, **kwargs)
            
        save_dir = repo_root / f"io_capture_{self.__class__.__name__.lower()}_{func.__name__.lower()}"
        
        # Create the directory (overwrites if exists, which is desired per new requirement)
        save_dir.mkdir(exist_ok=True, parents=True) # parents=True is good practice
        ...
        for i, arg_val in enumerate(args):
            if i < len(param_names):
                ...
                if isinstance(arg_val, torch.Tensor):
                    ...
                    inputs_dict[param_name] = arg_val
        ...
        BaseModelOutput = getattr(sys.modules.get("transformers.modeling_outputs", {}), "BaseModelOutput", None)

        def _handle_custom_output_type(item_key: str, item_value: Any) -> Union[torch.Tensor, bool, type(NotImplemented)]:
            if BaseModelOutput and isinstance(item_value, BaseModelOutput):
                if hasattr(item_value, 'last_hidden_state') and isinstance(item_value.last_hidden_state, torch.Tensor):
                    return item_value.last_hidden_state
            return NotImplemented

        none_outputs_for_metadata = {}
        bool_outputs_for_metadata = {}
        outputs_to_save_in_pt = {}

        items_to_process = []
        if isinstance(outputs_result, tuple):
            for i, output_item in enumerate(outputs_result):
                items_to_process.append((f"output_{i}", output_item))
        else:
            items_to_process.append(("output", outputs_result))

        for key, item_value in items_to_process:
            if item_value is None:
                none_outputs_for_metadata[key] = None
            elif isinstance(item_value, bool):
                bool_outputs_for_metadata[key] = item_value
            elif isinstance(item_value, torch.Tensor):
                outputs_to_save_in_pt[key] = item_value.detach().cpu()
            else:
                handled_item = _handle_custom_output_type(key, item_value)
                if isinstance(handled_item, torch.Tensor):
                    outputs_to_save_in_pt[key] = handled_item.detach().cpu()
                elif isinstance(handled_item, bool):
                    bool_outputs_for_metadata[key] = handled_item
                elif handled_item is NotImplemented:
                    raise TypeError(
                        f"Function '{func.__name__}' in class '{self.__class__.__name__}' returned an output "
                        f"'{key}' of unhandled type '{type(item_value).__name__}'. "
                        f"Please update '_handle_custom_output_type' in the 'save_io' decorator."
                    )
                else: 
                    raise TypeError(
                        f"Custom type handler for '{func.__name__}' output '{key}' "
                        f"returned an invalid type '{type(handled_item).__name__}'. Expected torch.Tensor or bool."
                    )
        ...
        output_metadata_content = {
            "none_outputs": none_outputs_for_metadata,
            "bool_outputs": bool_outputs_for_metadata,
            "saved_pt_info": {
                k: {"shape": list(v.shape) if hasattr(v, 'shape') else [],
                    "dtype": str(v.dtype) if hasattr(v, 'dtype') else type(v).__name__}
                for k, v in outputs_to_save_in_pt.items()
            }
        }
        with open(save_dir / output_metadata_file_name, "w") as f:
            json.dump(output_metadata_content, f, indent=2)
            
        print(f"Saved inputs, outputs (as .pt), and their metadata to {save_dir} for {func_identifier}")
